#!/bin/bash
#SBATCH --job-name=solver_scan_timing
#SBATCH --nodes=1
#SBATCH --output=solver_scan_timing_%j_%N.out
#SBATCH --ntasks=1
...
#
# Regional/120, both solvers changed, theta compiler fusion fixed in both arms.
set -Eeo pipefail
PHASE="startup"
trap 'rc=$?; if (( rc != 0 )); then echo "FAILED: exit ${rc}; phase=${PHASE}; output=${OUT_DIR:-Slurm stdout}" >&2; fi' EXIT
echo "solver_scan_timing starting: job=${SLURM_JOB_ID:-unset}, node=${SLURM_JOB_NODELIST:-unset}"
...
if [[ ! -r setup_amd_env.sh ]]; then
    echo "Missing ${REPO_ROOT}/setup_amd_env.sh." >&2
    exit 2
fi
set +e
...
EXPERIMENT_ROOT="${REPO_ROOT}/amd_scripts/solver_scan_fusion"
...
OUT_DIR="${REPO_ROOT}/amd_scripts/solver_scan_fusion_runs/amd_${SLURM_JOB_ID}"
PHASE="validated granule timing"
python "${EXPERIMENT_ROOT}/measure.py" --output "${OUT_DIR}"
echo "PASS: ${OUT_DIR}/TIMING_SUMMARY.md"
