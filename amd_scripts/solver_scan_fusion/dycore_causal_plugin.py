...
def make_program(program, spec, bounds):
    if spec.get("solver_scan_fusion") and "vertically_implicit_solver" in program.__name__:
        from solver_program import make_solver_program

        replacement = make_solver_program(program, spec, Path(os.environ["ICON4PY_CAUSAL_OUTPUT"]) / "programs")
        STATE["aliases"][replacement.__name__] = program.__name__
        return replacement
...
class ProgramPair:
    ...
    def __call__(self, *args, **kwargs):
        arm = "A" if case().get("solver_scan_fusion") and self.name == THETA else STATE["arm"]
        spec = arm_spec(case(), arm)
        if spec is None or not selected(self.name, spec):
            return self.native(*args, **kwargs)
        attribute = "baseline" if arm == "A" else "variant"
        checked_attribute = "baseline_checked" if arm == "A" else "checked"
...
def pytest_configure(config):
    ...
    require(
        Path(gt4py.__file__).resolve().parent == Path(os.environ["THETA_COMPILER_OVERLAY"]).resolve(),
        "Wrong GT4Py package loaded; expected the private compiler overlay.",
    )
    ...
    def options(program_name, backend_config, **descriptor):
        ...
        from theta_compiler import configure_options

        configure_options(program_name, case(), result)
        return result
    ...
    def folder(sdfg, code_objects, *args, **kwargs):
        ...
        if owner in STATE["programs"] or sdfg.name in STATE["aliases"]:
            for obj in code_objects:
                ...
                from theta_compiler import graph_evidence

                if owner == THETA or "vertically_implicit_solver" in owner:
                    audit["theta_graph"] = graph_evidence(sdfg)
