#!/bin/bash
#SBATCH --job-name=theta_compiler_timing
#SBATCH --nodes=1
#SBATCH --output=theta_compiler_timing_%j_%N.out
#SBATCH --ntasks=1
...
#
# Regional/120, native and Python-fused comparisons, timing only.
set -Eeo pipefail
PHASE="startup"
trap 'rc=$?; if (( rc != 0 )); then echo "FAILED: exit ${rc}; phase=${PHASE}; output=${OUT_DIR:-Slurm stdout}" >&2; fi' EXIT
echo "theta_compiler_timing starting: job=${SLURM_JOB_ID:-unset}, node=${SLURM_JOB_NODELIST:-unset}"
...
EXPERIMENT_ROOT="${REPO_ROOT}/amd_scripts/theta_shared_timing"
...
OUT_DIR="${REPO_ROOT}/amd_scripts/theta_shared_timing_runs/amd_${SLURM_JOB_ID}"
