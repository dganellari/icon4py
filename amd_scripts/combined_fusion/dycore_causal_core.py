...
def selected(name, case):
    return bool(case.keys() & {"order_width", "fusion", "vlb", "block", "block_2d", "compiler_fusion"}) and (
        case.get("scope") == "all" or name in case.get("targets", []) or name == case.get("target", THETA)
    )
