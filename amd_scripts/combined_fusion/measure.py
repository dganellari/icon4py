#!/usr/bin/env python3
"""Measure original versus combined theta and solver fusion on regional/120."""
...
from theta_compiler import SOLVERS, check_graphs, comparisons
...
def summarize(report):
    ...
    for metric in ("granule_device", "solvers_device", "theta_device", "granule_wall"):

        def value(block, metric=metric):
            if metric == "solvers_device":
                return sum(block["programs"][name] for name in SOLVERS)
        ...
        result["metrics"][metric] = dict(
            ...
            variant_ms=statistics.mean(timing["quartet_arm_ms"]["B"]),
...
def write_summary(out, results, platform):
    dump(out / "TIMING_SUMMARY.json", results)
    lines = [
        f"# {platform} regional/120 original versus combined fusion",
        "",
        "Positive saving means combined theta and solver fusion is faster. These are same-node granule measurements.",
    ...
    for name, result in results.items():
        for metric, item in result["metrics"].items():
            ...
            lines.append(
                ...
                f"{item['variant_ms']:.6f} | {item['saved_ms']:.6f} | [{lo:.6f}, {hi:.6f}] |"
            )
...
def main():
    ...
    status = dict(status="running", phase="compiler overlay", platform=args.platform)
    ...
    try:
        ...
        require(
            bool(cp.cuda.runtime.is_hip) == (args.platform == "amd"),
            "Requested platform does not match the GPU runtime.",
        )
        ...
        for index, spec in enumerate(comparisons()):
            ...
            dump(directory / "code_audit.json", check_graphs(spec, audits, args.platform))
            results[spec["name"]] = summarize(report)
            write_summary(out, results, args.platform)
