#!/bin/bash
#SBATCH --job-name=combined_fusion
#SBATCH --nodes=1
#SBATCH --output=combined_fusion_%j_%N.out
#SBATCH --ntasks=1
...
echo "combined_fusion starting: job=${SLURM_JOB_ID:-unset}, node=${SLURM_JOB_NODELIST:-unset}"
...
EXPERIMENT_ROOT="${REPO_ROOT}/amd_scripts/combined_fusion"
...
OUT_DIR="${REPO_ROOT}/amd_scripts/combined_fusion_runs/amd_${SLURM_JOB_ID}"
PHASE="validated granule timing"
python "${EXPERIMENT_ROOT}/measure.py" --platform amd --output "${OUT_DIR}"
